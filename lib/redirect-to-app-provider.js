      ...
      */
      var attemptRedirect = function ( path )
      {
        if(window.location.href.search('device_url=') >= 0){
          ...
          var urlSchemaCookie = getCookie('_app_hover_scheme');

          if (urlSchemaCookie !== undefined){
            var currentApps = decodeURI(urlSchemaCookie).split('|');
            if (currentApps !== undefined && currentApps.indexOf(urlSchema) >= 0){
              iOS9Redirect = true;
